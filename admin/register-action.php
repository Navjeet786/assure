	...
	$db = new Main;
		if (isset($_POST['action'])) {
		 	if ($_POST['action'] == 'register-admin') {
                ...
                # check user
              echo  $qry = "SELECT * FROM `admin` WHERE email = '".$email."'";
                $run = $db->select($qry);
	                if ($run->num_rows > 0) {
	                	...
	                	echo $query = "INSERT INTO `admin`(first_name,last_name,email,password,role,add_date)VALUES('".$fname."','".$lname."','".$email."','".$pwd."','Admin','".$date."')";
	                		if ($db->insert($query)) {
